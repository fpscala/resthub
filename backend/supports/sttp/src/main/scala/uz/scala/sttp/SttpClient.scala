...
object SttpClient {
  def apply[F[_]: Sync: SttpBackends.Simple, H](
    ...
    new SttpClient[F, H] {
      private val logger: Logger[F] = Slf4jLogger.getLoggerFromName[F]("uz.scala.sttp-client")
        ...
        for {
          ...
          _ <- logger.info(
            "HTTP Request\n" +
              ...
              s" -- Body: ${modifyRequestBodyForShow(reqBody.toString)}\n" +
              " - RESPONSE\n" +
  ...
  type StringBody[F[_]] = SttpClient[F, StringResponse]
  ...
  def string[F[_]: Sync: SttpBackends.Simple](
      apiUrl: URI,
      auth: SttpClientAuth,
    ): SttpClient.StringBody[F] =
    apply(apiUrl, auth, asString)
